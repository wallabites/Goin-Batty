...
public class Notes : MonoBehaviour
{
    ...
    // BOOLS
    public bool yesorno; // a bool is a true or false statement, a binary, think of it like a light switch
    // its either on or off no in between

    // OTHER COMMON VARIABLES
    public GameObject myGameObject; // we can reference any object in our scene
    // every item in the hierarchy is considered a gameobject
    public Notes mynotescript; // we can reference scripts as well as long as they are public
    public Rigidbody2D myRB2D; // we use rigidbodies to make gameobjects affected by Unity's physics engine 
    ...
    void Start()
    {
        ...
        //compenents in the inspector can be enabled and disabled
        ...
        //if  we don't want to drag and drop all variables into the inspector there are a few commands that will do this for us automatically but they do take processing power
        //generally we only do this when we have to, such as spawning the player
        myRB2D = GetComponent<Rigidbody2D>(); // this will get the Rigidbody but only if it is on the same object as our script
        ...
        transform.position = new Vector3(0, 0, 0); // this is x, y, and z in a 3D space
        // x is horizontal axis (walking)
        // y is vertical axis (jumping)
        // z is depth (also walking in 3D)
        transform.position = new Vector2(24, 128); //this line would teleport the game object 24 units horizontally and 128 units vertically
    ...
    void Update()
    {
        ...
        // player controls, enemy controls, pints, health, etc all happen inside teh update loop
        ...
        // the double equal sign is needed for this to work as it checks to see IF the statement is true or false
        // "yesorno = true" automatically sets the bool to be true
        if( number >= 10)
        {
            //we do something
                    }
    ...
    if(Input.GetAxis("Horizontal") > 0)
        {
            //move the player
            myRB2D.velocity = new Vector2(2, myRB2D.velocity.y); // we don't alter the y here so that the jump doesn't get affected
    ...
    if(Input.GetAxis("Horizontal") == 0)
        ...
        myRB2D.gravotyScale = 0; //this turns off gravity
        myRB2D.simulated = false; //this makes you invincible (things can pass through you)
        myRB2D.isKinematic = true; //when this is true the rigidbody only moves when the code tells it to
        ...
        if (yesorno == true)
        {
            //we say yes
            //the player lives
            //sometimes bools (of if statements) change too fast which makes for weird behavior
        }
    ...
    {
        //regular update is dependent of the frame of the game-- this means newer devices will play the game faster and older devices will play the game slower which is not ideal
        //FIXED UPDATE updates at a set interval which means all machines run the code at the same speed
        //we put physics code inside fixed update
    }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        //it could be an enemy, an animation, or music-- really anything we want
        ...
        //normally we check to see the tag of the collision so only the player can trigger the event
        //for the event to trigger one object needs to have a rigidbody (either the player or the object that has the trigger collider on it)
    }

    private void OnCollisionEnter2D(Collision2D collision)
    {
        //same as trigger except we don't pass through the collider and the collider is not marked as trigger
        //this will trigger events when we bump into a collider
    }
}
